...
export default function DashboardClient({ storeId }: { storeId: string }) {
  const { data, isLoading } = useQuery({
    queryKey: ['storeStatus', storeId],
    queryFn: () => fetchWithAuth(`/api/v1/store/${storeId}/status`).catch(() => ({ rooms: mockRooms })),
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Store #{storeId}</h1>
